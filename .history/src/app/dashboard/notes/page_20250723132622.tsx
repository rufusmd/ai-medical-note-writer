...
import { EditDelta, EditAnalysisResult } from '@/types/editTracking'; // NEW - Phase 4A
import {
    UserIcon,
    DocumentTextIcon,
...
export default function NotesPage() {
    ...
    // Load patients from Firebase (PRESERVED FROM YOUR EXISTING CODE)
    useEffect(() => {
        if (!user?.uid) {
    ...
    // Auto-select patient based on URL parameter (PRESERVED)
    useEffect(() => {
    ...
    // ENHANCED - Phase 4A note generation with edit tracking
        ...
        try {
            const response = await fetch('/api/generate-note-enhanced', {
                ...
                body: JSON.stringify({
                    ...
                    clinicalContext: {
                        emrSystem: clinicalContext.emr,
                        institution: clinicalContext.clinic === 'hmhi-downtown' ? 'HMHI Downtown' : 'Davis Behavioral Health',
                    ...
                    specialty: 'psychiatry',
                    userId: user.uid,
                    // NEW - Phase 4A: Request optimized prompts
            ...
            // NEW - Phase 4A: Start edit session tracking
            if (result.content) {
                const sessionId = await editTrackingService.createEditSession(
                    result.noteId || `note_${Date.now()}`,
                    selectedPatient.id,
                    user.uid,
                    {
                        emrSystem: clinicalContext.emr,
                ...
                setIsUsingEditableInterface(true); // Enable editable interface
            }
    ...
    // NEW - Phase 4A: Handle delta tracking
    const handleDeltaCapture = async (delta: EditDelta) => {
    ...
    // NEW - Phase 4A: Handle note saving with learning
    const handleNoteSave = async (content: string, deltas: EditDelta[]) => {
    ...
    // Copy note to clipboard (PRESERVED FROM YOUR EXISTING CODE)
    ...
    // Helper function to format last visit date (PRESERVED)
    ...
    // NEW - Toggle between static and editable note display
    const toggleEditableInterface = () => {
        setIsUsingEditableInterface(!isUsingEditableInterface);
        if (!isUsingEditableInterface && generatedNote && selectedPatient && user) {
            // Starting edit session
            editTrackingService.createEditSession(
                generatedNote.noteId || `note_${Date.now()}`,
                selectedPatient.id,
                user.uid,
                {
                    emrSystem: clinicalContext.emr,
                ...
                generatedNote.promptVersion || 'v1.0',
                generatedNote.generationTime || 0
            ).then(sessionId => {
                setCurrentEditSession(sessionId);
                setEditStartTime(Date.now());
            });
    ...
    // Render create mode with gorgeous stacked layout (PRESERVED + ENHANCED)
    if (isCreateMode) {
        return (
            <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
                <div className="max-w-7xl mx-auto px-6 py-8">
                    ...
                    {/* NEW - Learning Insights Panel */}
                    {showLearningInsights && editAnalytics && (
                        <div className="mb-8 bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-200 rounded-xl p-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <h4 className="font-medium text-indigo-800 mb-2">Detected Patterns</h4>
                                    {editAnalytics.patterns.length > 0 ? (
                                        <ul className="space-y-2">
                                            {editAnalytics.patterns.map((pattern, index) => (
                                                <li key={index} className="text-sm">
                                                    <span className="font-medium text-indigo-700">{pattern.description}</span>
                                        ...
                                        <p className="text-indigo-600 text-sm">No significant patterns detected</p>
                                    )}
                                </div>

                                <div>
                                    <h4 className="font-medium text-indigo-800 mb-2">AI Improvements</h4>
                                    {editAnalytics.promptSuggestions.length > 0 ? (
                                        ...
                                        <p className="text-indigo-600 text-sm">No specific improvements identified</p>
                                    )}
                                </div>
                            </div>
                    ...
                    {/* STACKED LAYOUT - Clinical Context Configuration at top (PRESERVED) */}
                    <div className="space-y-8">
                        ...
                        {/* Patient Selection and Transcript - Side by side (PRESERVED) */}
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            {/* Patient Selection (PRESERVED - ALL YOUR EXISTING CODE) */}
                            ...
                            {/* Transcript Input and Note Generation (PRESERVED + ENHANCED) */}
                            <div className="lg:col-span-2 space-y-6">
                                {/* Transcript Input (PRESERVED) */}
                                <div className="bg-white shadow-lg rounded-xl border border-gray-200 overflow-hidden">
                                ...
                                {/* ENHANCED - Generated Note Display with Editable Option */}
                                {generatedNote && (
                                    <div className="bg-white shadow-lg rounded-xl border border-gray-200 overflow-hidden">
                                        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-4 text-white">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    {/* NEW - Toggle Editable Interface */}
                                                    <button
                                                        onClick={toggleEditableInterface}
                                                        className={`flex items-center gap-2 px-3 py-1 rounded-md text-sm transition-colors ${isUsingEditableInterface
                                                                ? 'bg-yellow-500 text-yellow-900'
                                                                : 'bg-white/20 text-white hover:bg-white/30'
                                                            }`}
                                                        title="Toggle editable interface with AI learning"
                                                    >
                                                        <PencilIcon className="h-4 w-4" />
                                                        {isUsingEditableInterface ? 'Learning Mode' : 'Edit Mode'}
                                                    </button>
                                        ...
                                        <div className="p-6">
                                            {/* NEW - Conditional rendering: Editable vs Static */}
                                            {isUsingEditableInterface ? (
                                                <EditableNoteEditor
                                                    initialContent={generatedNote.content}
                                                    noteId={generatedNote.noteId || `note_${Date.now()}`}
                                                    patientId={selectedPatient?.id || ''}
                                                    clinicalContext={{
                                                        emrSystem: clinicalContext.emr,
                                                        institution: clinicalContext.clinic === 'hmhi-downtown' ? 'HMHI Downtown' : 'Davis Behavioral Health',
    ...
    // Default notes list view (PRESERVED)
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
            <div className="max-w-7xl mx-auto px-6 py-8">
                <div className="flex items-center justify-between mb-8">
