...
// Real implementation using your enhanced section detector
...
// Import your enhanced detector types
import type { EnhancedParsedNote, StandardizedSectionType, EnhancedDetectedSection } from '@/lib/note-processing/section-detector';
...
// Organized section groups matching your clinical workflow
...
const SMART_PRESETS = {
    'transfer-standard': {
        ...
        icon: Zap,
        selections: ['CURRENT_MEDICATIONS', 'HPI', 'REVIEW_OF_SYSTEMS', 'PSYCHIATRIC_EXAM', 'QUESTIONNAIRES_SURVEYS', 'MEDICAL', 'RISKS', 'ASSESSMENT_AND_PLAN', 'PSYCHOSOCIAL', 'SAFETY_PLAN', 'FOLLOW_UP']
    },
    'medication-focused': {
...
export default function ImprovedSelectiveUpdater({
    ...
    useEffect(() => {
        const getDefaultSelections = (visitType: string): string[] => {
            switch (visitType.toLowerCase()) {
                ...
                case 'transfer of care':
                    return SMART_PRESETS['transfer-standard'].selections;
                case 'follow-up':
                case 'followup':
                    return ['HPI', 'REVIEW_OF_SYSTEMS', 'PSYCHIATRIC_EXAM', 'QUESTIONNAIRES_SURVEYS', 'RISKS', 'ASSESSMENT_AND_PLAN', 'MEDICATIONS_PLAN', 'SAFETY_PLAN', 'FOLLOW_UP'];
                case 'intake':
                case 'initial':
                    return SMART_PRESETS['comprehensive'].selections;
                default:
                    return SMART_PRESETS['minimal'].selections;
            }
        };

        const defaultSelections = getDefaultSelections(clinicalContext.visitType);

        // Only select sections that actually exist in the parsed note
        const availableSections = parsedNote.sections.map(s => s.type);
        const validSelections = defaultSelections.filter(selection =>
            availableSections.includes(selection as StandardizedSectionType)
        );
        ...
        updateParent(newSelectedSections, availableSections);
    }, [parsedNote, clinicalContext.visitType, onUpdateConfigChange]);

    // Update parent component with current selections
    const updateParent = (selected: Set<string>, allSections: string[]) => {
        const sectionsToUpdate = Array.from(selected);
        const sectionsToPreserve = allSections.filter(s => !selected.has(s));
    ...
    const toggleSection = (sectionType: string) => {
        ...
        updateParent(newSelected, allSections);
    };
    ...
    const applyPreset = (presetId: string) => {
        ...
        if (preset) {
            ...
            updateParent(newSelected, availableSections);
        }
    };
    ...
    const getSection = (sectionType: string): EnhancedDetectedSection | undefined => {
    ...
    const clearAll = () => {
        ...
        setSelectedPreset('');
        const allSections = parsedNote.sections.map(s => s.type);
        updateParent(new Set(), allSections);
    };
    ...
    const updateSummary = {
        ...
        standardizedSections: parsedNote.parseMetadata.standardizedSections
    };
    ...
    return (
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-purple-500 to-pink-600 px-6 py-4">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 text-white">
                        ...
                        <div>
                            <h3 className="text-lg font-semibold">Enhanced Transfer of Care Selector</h3>
            ...
            <div className="p-6 space-y-6">
                {/* Success indicator for enhanced detector */}
                ...
                <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                        {Object.entries(SMART_PRESETS).map(([key, preset]) => {
                            ...
                            // Count how many sections from this preset are actually available
                    ...
                    <div className="space-y-4">
                        {Object.entries(SECTION_GROUPS).map(([groupId, group]) => {
                            ...
                            const availableSections = group.sections
                                ...
                                .filter(Boolean) as EnhancedDetectedSection[];
