...
export class ProviderManager {
    ...
    // Clinical note generation with custom prompts
    async generateNoteWithCustomPrompts(
        request: NoteGenerationRequest,
        systemPrompt: string,
        userPrompt: string
    ): Promise<NoteGenerationResponse> {
        ...
        try {
            processingSteps.push('Starting clinical note generation with custom prompts');
            ...
            if (primaryProvider) {
                ...
                try {
                    const result = await this.generateWithCustomPromptsTimeout(primaryProvider, request, systemPrompt, userPrompt);
            ...
            if (this.config.enableFallback) {
                ...
                if (fallbackProvider) {
                    ...
                    try {
                        const fallbackStartTime = Date.now();
                        const result = await this.generateWithCustomPromptsTimeout(fallbackProvider, request, systemPrompt, userPrompt);
            ...
            return {
                success: false,
                error: {
                    code: error.code || 'CLINICAL_PROVIDER_ERROR',
                    message: error.message || 'Failed to generate clinical note with any provider',
                    details: {
                        originalError: error,
                        processingSteps,
                        totalDuration,
    ...
    private async generateWithCustomPromptsTimeout(
        provider: AIProvider,
        request: NoteGenerationRequest,
        systemPrompt: string,
        userPrompt: string
        ...
        return new Promise((resolve, reject) => {
            ...
            // Call provider with custom prompts
            provider.generateNoteWithPrompts(request, systemPrompt, userPrompt)
                .then(result => {
                    clearTimeout(timeout);
                    resolve(result);
                })
