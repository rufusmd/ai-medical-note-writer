...
export class NoteValidationUtility {
    ...
    private static validateEMRFormatting(noteContent: string, context: ClinicalContext): {
        ...
        if (context.clinic === 'Davis Behavioral Health') {
            ...
            // ENHANCED: Check for Epic wildcards - more comprehensive
            if (noteContent.includes('***')) {
                errors.push('Epic wildcards (***) detected in Credible EMR note');
                penalties += 25; // Increased penalty
            }

            // Check for single and double asterisks used as placeholders
            const asteriskMatches = noteContent.match(/\*+/g);
            if (asteriskMatches) {
                errors.push(`Asterisk placeholders detected in Credible EMR note: ${asteriskMatches.join(', ')}`);
                penalties += 20;
            }
            ...
            // Check for incomplete sections that suggest placeholders
            const incompleteSections = [
                /Mental Status Exam:\s*\*+/i,
                /Assessment:\s*\*+/i,
                /Plan:\s*\*+/i,
                /MSE:\s*\*+/i
            ];

            incompleteSections.forEach(pattern => {
                if (pattern.test(noteContent)) {
                    errors.push('Incomplete clinical sections with placeholder syntax detected');
                    penalties += 30; // High penalty for incomplete clinical content
                }
            });
    ...
    /**
     * Clean Epic syntax from text (emergency fallback) - ENHANCED VERSION
     */
    static cleanEpicSyntax(noteContent: string): string {
        ...
        // Remove all asterisk wildcards (*, **, ***, etc.)
        cleanedContent = cleanedContent.replace(/\*+/g, '');

        // Remove other Epic placeholder patterns
        cleanedContent = cleanedContent.replace(/\$\$\$/g, '');
        cleanedContent = cleanedContent.replace(/%%%/g, '');
        ...
        cleanedContent = cleanedContent.replace(/DotPhrase/gi, 'phrase');

        // Clean up common placeholder patterns for clinical sections
        cleanedContent = cleanedContent.replace(/Mental Status Exam:\s*\*+/gi, 'Mental Status Exam: Alert and cooperative, well-groomed appearance');
        cleanedContent = cleanedContent.replace(/Assessment:\s*\*+/gi, 'Assessment: Clinical impression based on presentation');
        cleanedContent = cleanedContent.replace(/Plan:\s*\*+/gi, 'Plan: Treatment plan to be determined');
        cleanedContent = cleanedContent.replace(/MSE:\s*\*+/gi, 'Mental Status Exam: Normal mental status examination');

        // Remove any remaining standalone asterisks or placeholder symbols
