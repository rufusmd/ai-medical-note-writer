...
const PatientManagement: React.FC = () => {
    ...
    // Handler for when a new patient is created
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded-lg p-6 w-full max-w-2xl">
                        ...
                        <div className="space-y-4">
                            ...
                            {selectedPatient.mrn && <div><strong>MRN:</strong> {selectedPatient.mrn}</div>}
                            {selectedPatient.primaryDiagnosis && <div><strong>Diagnosis:</strong> {selectedPatient.primaryDiagnosis}</div>}
                            <div><strong>Clinic:</strong> {getClinicName(selectedPatient.primaryClinic)}</div>
                            <div><strong>EMR:</strong> {selectedPatient.preferredEMR.toUpperCase()}</div>
                            <div><strong>Notes Generated:</strong> {selectedPatient.noteCount}</div>
                            {selectedPatient.averageNoteQuality && (
                                <div><strong>Average Quality:</strong> {selectedPatient.averageNoteQuality.toFixed(1)}/10</div>
                            )}
                        </div>
                    </div>
                </div>
